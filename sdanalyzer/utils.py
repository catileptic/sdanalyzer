...
def get_db_path():
    config_dir = get_or_create_config_path()
...
def check_vt(hashes):
    ...
    """
    apikey = ""
    url = "https://www.virustotal.com/partners/sysinternals/file-reports?apikey={}".format(
        apikey
    )
    items = []
...
def convert_x509_name(name):
    ...
    return "/".join(
        ["{}={}".format(types[attr], name.native[attr]) for attr in name.native]
    )
...
def get_urls(apk):
    ...
    for dex in apk.get_all_dex():
        res += re.findall(
            rb"http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+",
            dex,
        )
...
def extract_apk_infos(apk_path, rules):
    ...
    if len(apk.get_certificates()) > 0:
        ...
        res["certificate"]["not_before"] = cross_strftime(
            cert["tbs_certificate"]["validity"]["not_before"].native
        )
        res["certificate"]["not_after"] = cross_strftime(
